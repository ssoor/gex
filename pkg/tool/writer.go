...
var (
	toolsGoTemplate = template.Must(template.New("tools.go").Parse(`// Code generated by github.com/ssoor/gex. DO NOT EDIT.
...
import (
{{- range $t := .Tools}}
	_ "{{$t}}"{{if $t.Global}} // Global Tools{{end}}
{{- end}}
)
...
//
{{- range $t := .Tools}}
//go:generate go {{if $t.Global}}get -v{{else}}build -v -o=./bin/{{$t.Name}}{{end}} {{if $.ManagerType.Vendor}}./vendor/{{end}}{{$t}}
{{- end}}
`))
)
